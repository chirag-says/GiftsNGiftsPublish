 ...
 * 
 * SAFE IMAGE FORMATS ALLOWED:
 * - JPEG/JPG: Bitmap, no script support
 * - PNG: Bitmap, no script support  
 * - WebP: Modern format, no script support
 * 
 * DOCUMENT FORMATS ALLOWED (for KYC/Documents):
 * - PDF: Document format for KYC uploads
 */
...
/**
 * SECURITY: Pre-validation file filter (before any processing) - IMAGES ONLY
 */
...
/**
 * SECURITY: Pre-validation file filter for DOCUMENTS (allows PDFs for KYC/onboarding)
 */
const documentValidationFilter = (req, file, cb) => {
  const ext = path.extname(file.originalname).toLowerCase();
  const originalName = file.originalname.toLowerCase();

  // Check for dangerous file patterns FIRST (except PDF)
  for (const pattern of DANGEROUS_PATTERNS) {
    if (pattern.test(originalName)) {
      return cb(new Error(`File type not allowed: Potentially dangerous file detected`), false);
    }
  ...
  // Check declared MIME type - includes PDFs
  if (!ALLOWED_DOCUMENT_MIME_TYPES.has(file.mimetype)) {
    return cb(new Error(`Invalid file type: ${file.mimetype}. Only JPEG, PNG, WebP images and PDF documents are allowed.`), false);
  }

  // Check file extension - includes PDF
  if (!ALLOWED_DOCUMENT_EXTENSIONS.has(ext)) {
    return cb(new Error(`Invalid file extension: ${ext}. Allowed extensions: .jpg, .jpeg, .png, .webp, .pdf`), false);
  }
...
/**
 * Standard upload configuration with strict security (IMAGES ONLY)
 */
const uploadConfig = {
  storage,
...
/**
 * Document upload configuration (allows PDFs for KYC/onboarding)
 */
const documentUploadConfig = {
  storage,
  fileFilter: documentValidationFilter,
  limits: {
    fileSize: 5 * 1024 * 1024,    // 5MB max file size
    files: 10,                     // Max 10 files per request
    fieldNameSize: 100,            // Max field name length
...
/**
 * DOCUMENT UPLOAD: For KYC documents (allows PDFs)
 * Use this for onboarding document uploads
 */
...
/**
 * SECURITY: Magic byte verification for documents (includes PDF)
 */
export const verifyDocumentMagicBytes = async (req, res, next) => {
  if (!req.file && !req.files) {
    return next();
  }
  ...
  try {
    for (const file of files) {
      ...
      if (!ALLOWED_DOCUMENT_MIME_TYPES.has(detectedType.mime)) {
        if (file.path) fs.unlinkSync(file.path);
        return res.status(400).json({
          success: false,
          message: `File content does not match an allowed type. Detected: ${detectedType.mime}. Allowed: JPEG, PNG, WebP, PDF.`
        });
      }
    }
    ...
    console.error('Document magic byte verification error:', error);
    if (req.file?.path && fs.existsSync(req.file.path)) {
      fs.unlinkSync(req.file.path);
    }
    return res.status(500).json({
