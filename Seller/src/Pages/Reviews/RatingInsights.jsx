...
function RatingInsights() {
  ...
  const fetchInsights = async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/seller-panel/reviews/insights?period=${period}`);
                  ...
                  return (
                    <div key={i} className="group">
                      ...
                      <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-1000 ${cat.rating >= 4 ? 'bg-emerald-500' : cat.rating >= 3 ? 'bg-amber-500' : 'bg-rose-500'
                            }`}
